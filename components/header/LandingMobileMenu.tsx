    ...
    <div className="flex flex-col items-end justify-center lg:hidden w-full">
      ...
      >
        <AnimatePresence mode="wait" initial={false}>
          {isOpen ? (
            <motion.span key="close" variants={iconVariants} initial="initial" animate="animate" exit="exit" className="absolute">
              <X size={26} />
            </motion.span>
          ) : (
            <motion.span key="open" variants={iconVariants} initial="initial" animate="animate" exit="exit" className="absolute">
              <Menu size={26} />
            </motion.span>
          )}
        </AnimatePresence>
          ...
          >
            {landingNavMenu.map((item, index) => (
              <div key={index}>
                <a
                  href={item.href}
                  className="font-poppins font-light text-black flex flex-col items-center py-3 shadow-xs transition-colors hover:text-alira"
