  ...
  return (
    <nav className={`lg:block hidden relative z-10 py-3 w-full ${isScrolled ? "bg-transparent" : "bg-white/10 backdrop-blur-xl rounded-full border border-white/30"}`}>
      {navbarMenu.map((item, index) => {
        ...
        return (
          <Link key={index} href={item.href} className={`font-poppins md:px-8 px-2 transition-all ${ isScrolled ? isActive ? "text-alira font-bold underline" : "text-black" : isActive ? "text-white font-bold" : "text-white font-light" }`}>
            {item.name}
          </Link>
        )
      })}
