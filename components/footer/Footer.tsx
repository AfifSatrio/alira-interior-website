import Image from "next/image";

const Footer = () => {
  return (
    <div className="pt-10 mx-auto w-full flex flex-col bg-white text-black">
        <div className="grid grid-cols-[1fr_2fr_1fr] mx-15">
            <Image src={"/logo/footer-logo.svg"} alt={"footer logo"} width={150} height={100} />
            <div className="flex flex-col font-poppins gap-2">
                <h1 className="font-bold text-lg">Punya Pertanyaan?</h1>
                <span className="font-light text-sm">Sampaikan kepada kami atau minta konsultasi seputar desain interior</span>
            </div>
            <div className="flex flex-col font-poppins gap-2">
                <h1 className="font-bold text-lg">
                    Hubungi Kami
                </h1>
                <span className="font-light text-sm">0823-2693-1783</span>
                <span className="font-light text-sm">info@alirainterior.com</span>
            </div>
        </div>
        <span className="font-light text-sm font-poppins mt-10 text-center mb-5">©2026 Copyright Alira Interior. All rights Reserved.</span>
    </div>
  )
}

export default Footer